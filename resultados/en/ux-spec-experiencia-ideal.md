# UX Spec — Experiencia Ideal (lead capture + SEO) · Sports World

| Field | Value |
|---|---|
| Version | v1.0 (derived and corrected from 01_UX_Specification_v4_2_10.docx) |
| Date | 2026-06-10 |
| Authors | Product · Design · Engineering · QA (co-authorship pending sign-off) |
| Status | In review |
| Output stack | **Next.js + React + TypeScript + Tailwind**, SSR/ISR rendering, headless CMS (signed: contract Annex Two I.3.b) |
| Handoff tool | `[TO BE DEFINED — link Figma inspect / Zeplin]` |
| Single source of truth (data/rules) | sw_experiencia_ideal_demo_v6_FINAL.jsx + this document |

> **Method note.** **Content-complete** document: it combines the strategic/UX layer of the `ux-spec` standard (§1–§10) with the **1:1 transfer** of the ENTIRE original technical document (Parts 1–6 + Appendices A–H: 144 sections and 24 tables, see "Technical Part"). Nothing from the original is left out; the strategic layer adds rationale, personas, journey, tokens, POUR accessibility, conversion, acceptance criteria, and metrics.
...
2. It is outside the top 100 for searches like "yoga cerca de mí" → **one dedicated page per class**: 51 adult classes (7 Les Mills + 44 regular) + the FitKidz hub with 34 children's activities (Part 3).
...
|---|---|
| **Single guided questionnaire** (adaptive, 15–21 questions) instead of a short form | It is an **interactive tool of value** (an "ideal experience" calculator): the user provides data in exchange for a personalized recommendation, which mitigates the bounce rate of long forms. *Risk:* it is still long → drop-off is measured per question (see §10) and progressive profiling is evaluated if abandonment exceeds the threshold. |
...
| --- | --- | --- |
| **Discover** | The 155 indexable pages (Part 3): 49 clubs · 51 classes (7 Les Mills signature + 44 regular) · 5 goal hubs (`/perfiles/…`) · the `/bajar-de-peso/` hub (YMYL) · 10 amenities · FitKidz · Personal Training · 10 individual-training pages · 6 membership pages · 20 journal articles · Home | **80,000 → 160,000 visits/month** (the indexable surface IS the lever) |
...
The exact technical phases (welcome · questionnaire · loading · result · contact_capture · schedule · briefing · error) and every branch live in **§3** and **§4**.

### 2.3 Consumer-journey insights that inform the design
...
| The review-check of the specific club decides the conversion | Club page (Part 3) with real photos, schedules, classes, reviews and amenities; Club Ideal card with verifiable data (Rule 42) |
...
## 3. Flows and Transition Diagram
...
**Safety filter (YMYL):** before building Block 3 (classes), the engine applies the **hard contraindication filter** (5 conditions: injury, cardiovascular, pregnancy, postpartum, bariatric). Contraindicated classes never appear. Full detail in the technical document (Rule 14b). 

---

## 4. Specification per Screen / Component

### 4.1 SEO topic hub (e.g. `/bajar-de-peso/`)
...
### 4.2 Questionnaire (`questionnaire`, Q1–Q19)

- **Purpose:** qualify and personalize; collect the lead's data.
- **Structure (official questionnaire):** **15 base questions** always visible (Q1–Q10, Q12, Q13, Q14, Q15, Q16) + **6 conditionals**: **Q11** (if Q10=pausa), **Q12b** (if Q2=Mujer), **Q14b** (children <12), and **Q17–Q19** (if Q4 includes Bajar de peso). Actual total **15–21** depending on the path (see the normative count table in the Technical Part).
...
### 4.5 Schedule and Brief (`schedule`, `briefing`)

- `schedule`: day/time selection; "Volver" returns to `contact_capture`.
- `briefing`: the Asesor's brief (10 sections, 5 generated by the LLM) with safety flags. Detail in the technical document (Appendix G).

---

## 5. Edge Case Matrix and Conditional States
...
| Exactly 1 viable class remains (audit L8; real at pabellón-bosques and triángulo-tecamachalco for pregnancy) | Filters leave 1 class | Single card + Personal Training as the second slot | "Esta clase encaja contigo; tu Asesor complementa el resto en la visita." |
...
| `loading` phase (LLM call) — NFR (audit L4) | Always | Immediate skeleton (0s); "Estamos armando tu experiencia…" message at ~5s; timeout with retry at ~15s (Rule 39 fallback if the retry fails) | "Esto está tomando más de lo normal. Reintentar." |
| Network error on `contact_capture`/`schedule` (audit F13) | Submit fails | Data retained on the client; button switches to "Reintentar" without losing what was typed; after 2 failures, offers WhatsApp/BES as an alternate channel | "No pudimos enviar tus datos. Reintentar." |
...
### 10.2 Lead scoring and routing

Relocated to `anexo-ingenieria-crm.md` (audit R14: CRM/sales logic with no UI behavior, `[SUPUESTO]` weights).

### 10.3 Progressive profiling (recommendation)
...
## Appendix — Open questions for the client (data; audit 2026-06-11)
...
## Appendix — Traceability with the technical document

This spec **does not replace** the engineering rules; it organizes them under the UX standard. The following are preserved and referenced: the Q1–Q19 questionnaire (Rule 18/19), the 6-subgroup model (Q4 mapping), the YMYL contraindication filter (Rule 14b), contact capture (Rule 32b), the Asesor brief (Appendix G) and the single LLM call (Appendix H) from the document `01_UX_Specification_v4_2_10.docx`.

---

---

# Technical Part — 1:1 transfer of the original document

> Faithful and complete transcription of `01_UX_Specification_v4_2_10.docx` (all headings, paragraphs and tables, in their original order). **Tables with merged cells** are rendered with the cell in its first column and the rest of the span left blank. Where the `.docx` contains **empty fields** in the source, they appear blank (nothing is invented).

> ⚠️ **Areas affected by corruption of the SOURCE `.docx` — reconstruction status.** The source file contained text broken mid-word and empty Word fields. Reconstructed in this pass (actual severity: medium, almost everything recoverable):
> - **Brand (Part 1 / Rule 8):** ✅ reconstructed → "premium fitness" / FitKidz "premium family fitness".
> - **Rule 13 — Schema markup:** ✅ reconstructed with standard schema.org types (confirm with engineering).
> - **Rule 2 — desktop labels:** ✅ reconstructed; the shortened labels (480–1023 and <480) were empty → to be defined with design.
> - **Rule 29 — contextual menu tags:** minor; reconstructible from Rules 26–31.
> - **Part 3 — Information Architecture diagram:** ❌ it was an **image**; not recoverable as text (the real IA lives in the "Page inventory"). Requires re-exporting from the `.docx` or redrawing the diagram.
> Document in **DRAFT** until these reconstructions are confirmed + the diagram is re-exported.
...
The site is designed and engineered mobile-first as a methodology, not as a responsive afterthought. Every layout, interaction, and rule in this specification is to be implemented starting from the mobile viewport and progressively enhanced upward. The ONLY breakpoint system is the token system (DESIGN.md): mobile 360 · tablet 768 · laptop 1024 · desktop 1440 (audit C11; the legacy template's 720px media query must migrate to 768). The opposite - designing desktop and "making it responsive" later- is non-conformant.
...
The **ACSM technical prescriptions per subgroup** (sets, reps, %1RM, rests, equipment, DOIs) live in `anexo-clinico.md` §2 (owner: MD validation; audit R6 — the text itself admits "not shown to the user" and the Part 2 scope boundary excludes them from the behavior spec).
...
Three official families (Fuerza y desarrollo muscular · Cardio y resistencia · Entrenamiento acuático), 6 sub-classes each, mapped to the 6 Q4 goals. The full detail (mapping tables and `clínico`/`inferido` statuses) lives in `anexo-clinico.md` §3 (audit R9 — data pending client confirmation). The aquatic block activates when Q6 = "En la alberca"/"Ambas" and the resolved club has a pool (Rule 39).

##### Questionnaire application flow — Experiencia Ideal + lead summary (per `sw_experiencia_ideal_demo_v6_FINAL.jsx`)
...
**1. Questionnaire flow (`getQuestions`).** 15 base + 6 conditional (see the normative table). Triggers: Q11 if Q10 = "Regreso después de una pausa"; Q12b if Q2 ≠ "Hombre" (audit L1 — includes "Prefiero no mencionarlo", with neutral phrasing); Q14b if Q14 ∈ {"Yo y mis hijos","La familia completa"}; Q17/Q18/Q19 if Q4 includes "Bajar de peso". Gender conjugation in Q3, Q13, Q14 when Q2 = Mujer.
...
| 480 to 1023 px (tablet / large mobile) | [shortened labels — came EMPTY in the source; to be defined with design] |
| Below 480 px (small mobile) | [icons / minimal labels — came EMPTY in the source; to be defined with design] |
...
| Total signed pages (Workstream B scope) | 155 (145 contractual + 10 individual-training, audit M4 — spec precedence) |
...
Research basis: see `research_contraindicaciones_audit.md` (protocol v2, 9 professional sources, epistemic labels [QUOTED]/[DERIVED]/[INFERRED]) — audit R3. Sports-medicine MD validation is **required (blocking gate)** before production YMYL deployment (see open dependencies).
...
| Q12b | ¿Estás embarazada o en posparto reciente? | Single-select, conditional (**visible unless Q2 = Hombre** — includes "Prefiero no mencionarlo"; audit L1: gender privacy cannot remove the medical screening; with "Prefiero no mencionarlo" the neutral phrasing "¿Aplica para ti alguna de estas situaciones?" is used) | Sí, embarazo · Sí, posparto reciente (últimos 6 meses) · No |
...
> Clarification (C-audit M6): "optionals" = **weight-path conditionals**: they appear only if Q4 = Bajar de peso, but **once on that path they are mandatory** (they cannot be skipped). The "required" type descriptor is correct.
...
| Goal hub — Ganar Fuerza | Q4 pre-marks "Aumentar masa muscular" (audit L7: the hub speaks of strength, not explosive power). | |
...
> **Normative question-count table (closes C5):**
...
> **Bridge table — technical protocol (ACSM fichas) ↔ official name (closes C5):**
>
...
| State | Questionnaire | Contextual menu |
| --- | --- | --- |
| No questionnaire | 15 questions, Q13 pre-marked Solo/Sola and Q4 pre-marked per subpage, both editable (18 if weight loss) | Diseña tu experiencia · Agenda tu visita guiada (audit L6: "Tu rutina individual" only exists once a result has been generated) |
...
- Health data (weight, height, medical conditions, medication use) requires additional, specific consent. **UI moment (audit L2):** the specific health-data consent is captured as a mandatory checkbox ON the Q12 screen, BEFORE the user can answer Q12/Q12b/Q17/Q18 — not after. The pre-result health-disclaimer modal is additional, not a substitute.
...
### Appendix E - Brand Voice and Tone (contract)

The complete Brand Voice Guide (approved/forbidden vocabularies, 5 writing rules, verbatim prohibitions from the system prompt — including "plan" → "Experiencia Ideal" —, hooks per Q3, before/after, connector rules ≤15 words, intent-lines ≤18, plan-argument ≤45, infrastructure ≤55) lives in `anexo-contenido-prompts.md` as the single copy (audit R10–R11; the previous duplication caused drift C13). **Contract that remains here:** the only free-generation zones for the LLM are hook, plan_argument, intent_line, infrastructure_argument and the 2 class connectors, each with its word limit; every factual statement comes from the backend/fichas; mandatory forbidden-vocabulary lint on the output (F9).
...
.hero__kicker { font-size:.6875rem; letter-spacing:.22em; text-transform:uppercase; color:#C81E20; font-weight:700; margin-bottom:.5rem; } /* audit C1: #E6282A 4.47:1 fails AA at 11px */
...
.club__kicker { font-size:.625rem; letter-spacing:.22em; text-transform:uppercase; color:var(--white); opacity:.85; font-weight:700; margin-bottom:.375rem; } /* audit C1: red/black 3.78:1 fails */
...
.block__action { font-size:.75rem; color:#C81E20; font-weight:700; text-decoration:none; border-bottom:1px solid #C81E20; align-self:flex-start; min-height:44px; display:inline-flex; align-items:center; } /* audit L3+C1: target ≥44px, red AA */
...
.cta__button { display:inline-block; background:var(--red); color:var(--white); font-size:1.1875rem; font-weight:700; padding:.875rem 2.25rem; border:none; cursor:pointer; border-radius:4px; text-decoration:none; } /* audit C1: white/red passes AA only ≥18.66px bold → 19px */
